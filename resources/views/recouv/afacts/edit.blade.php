...
{!! Form::model($afact, ['method' => 'PATCH','route' => ['afacts.update', $afact->id]]) !!}
...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9">
        <div class="EnteteContent">
            ...
            <div class="row">
                <input type="text" name="id" id="id" value="{{$afact->id}}" hidden>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="form-group row">
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <div>
                                    <select name="ReceptionId" id="ReceptionId">
                                        ...
                                        <option value="{{$Recep->id}}" {{ ($Recep->id==$recep->id) ? 'selected' : ''}} {{ (old('ReceptionId')==$Recep->id) ? 'selected' : ''}}> {{$Recep->Reference}}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <input class="form-control" name="Reference" value="{{old('Reference',$afact->Reference)}}" type="text">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input class="form-control datepicker" name="DateFacture" value="{{old('DateFacture',\Carbon\Carbon::parse($afact->DateFacture)->format('d/m/Y'))}}" type="text" required>
                                </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input class="form-control datepicker" name="DateEcheance" value="{{old('DateEcheance',\Carbon\Carbon::parse($afact->DateEcheance)->format('d/m/Y'))}}" type="text" required>
                                </div>
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="overflow-y: scroll;max-height:200px;">
                <table class="table table-bordered " id="TableOfData" style="font-size:90%;">
                    ...
                    <tbody id="DetailsUnites">
                        @if($afact->produits->count()>0)
                        @foreach ($afact->produits as $afact_produit)
                        @foreach ($afact->reception->produits as $recep_produit)
                        @foreach ($afact->reception->commande->produits->where('id',$recep_produit->id) as $cmde_produit)

                        @if($afact_produit->pivot->UniteId==$cmde_produit->pivot->UniteId)

                        @php
                        $Quantite= $recep_produit->pivot->Qte - $recep_produit->pivot->QteFacture + $afact_produit->pivot->Qte;
                        @endphp
                        
                        <tr class='PUnite{{$recep_produit->pivot->UniteId}}{{$recep_produit->id}}'>
                            <td>{{$cmde_produit->Libelle}} {{$recep_produit->pivot->Qte}} {{$recep_produit->pivot->QteFacture}} {{$afact_produit->pivot->Qte}}</td>
                            ...
                            <td hidden><input class='text-right form-control' list='PUnite{{$cmde_produit->pivot->UniteId}}{{$cmde_produit->id}}' name='Produit[]' min='0' value="{{old('Produit',$recep_produit->id)}}" type='number' />
                            </td>

                            <td hidden><input class='text-right form-control' list='PUnite{{$cmde_produit->pivot->UniteId}}{{$cmde_produit->id}}' name='Unite[]' min='0' value="{{old('Unite',$recep_produit->pivot->UniteId)}}" type='number' />
                            </td>
                        </tr>
                        @break
                        @endif
...
<div class="form-group" style="float:right;margin:15px;">
    <a href="{{url('/recouv/afacts')}}" class="btn btn-success btn-sm"><span class="dark bolder"><i class="glyphicon glyphicon-list"></i> Liste des factures</span></a>
