        ...
        <div class="col-sm-12 col-xs-12 col-md-6 col-lg-6">
            ...
            <div class="btn-group dropleft" style="float: right;">
                ...
                <ul class="dropleft dropdown-menu dropdown-inverse">
                    @can('createafact')
                    <li>
                        <a href="{{ route('afacts.create') }}" style="vertical-align: inherit;"><span class="glyphicon glyphicon-plus blue"></span> Nouvelle facture</a>
                    </li>
                    @endcan
                        ...
                        <table class="table table-bordered table-stripped data-table" style="font-size:80%;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">{{ \Carbon\Carbon::parse($row->DateReception)->format('d/m/Y')}}</td>
                                    <td>{{ $row->Reference }}</td>
                                    <td class="text-center">{{ \Carbon\Carbon::parse($row->DateEcheance)->format('d/m/Y')}}</td>
                                    <td>@if( $row->reception) {{ $row->reception->commande->fournisseur->Nom }} @endif</td>
                                    <td>@if( $row->reception){{ $row->reception->commande->Reference}} @endif</td>
                                    <td>@if( $row->reception) {{ $row->reception->Reference }} @endif</td>
                                            ...
                                            <ul class="dropleft dropdown-menu dropdown-inverse">
                                                <li>
                                                    <p style="min-width:250px;margin:0 0 0px;">
                                                        <a href="{{ route('afacts.show',$row->id) }}" style="padding-bottom:4px;" class="btn btn-white btn-xs bolder"><span class="ace-icon fa fa-eye dark"></span> Aperçu</a>

                                                        @can('editafact')

                                                        <a href="{{ route('afacts.edit',$row->id) }}" style="padding-bottom:0px;" class="btn btn-warning btn-xs bolder"><span class="glyphicon glyphicon-edit"></span>
                                                            Modifier</a>
                                                        ...
                                                        <a href="{{ route('apaiements.facture',$row->id) }}" style="padding-bottom:0px;" class="btn btn-info btn-xs bolder"><span class="icon-money"></span>
                                                             Paiement</a>
                                                        @endif
                                                        @endcan
                                                        ...
                                                        <!-- {!! Form::open(['method' => 'DELETE','route' => ['afacts.destroy',
...
<script type="text/javascript">
    ...
    $('.show_confirm').click(function(event) {
        ...
        $("#SubmitForm").attr('action', '/recouv/afacts/' + id);
