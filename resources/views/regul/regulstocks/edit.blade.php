...
{!! Form::model($regulstock, ['method' => 'PATCH','route' => ['regulstocks.update', $regulstock->id]]) !!}
...
<div class="EnteteContent">
    <div class="row">
        ...
        <div class="col-sm-12 col-xs-12 col-md-6 col-lg-6 text-right styleAction">
            Modification d'une régularisation stock
        </div>
    </div>
    <hr class="hrEntete">

    <div class="form-group row">
    <input type="text" name="id" id="id" value="{{$regulstock->id}}" hidden>
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="overflow-y: scroll;max-height:180px;">
                    <table class="table table-bordered " id="TableOfData" style="font-size:90%;">
                        ...
                        <tr class='PUnite{{$regulstock->UniteId}}{{$regulstock->ProduitId}}'>
                                <td>{{$regulstock->produit->Libelle}}</td>
                                <td>{{$regulstock->unite->Nom}}</td>
                                <td hidden><input class='text-right form-control' name='Produit[]' value='{{$regulstock->ProduitId}}' type='number'></td>
                                <td hidden><input class='text-right form-control' name='Unite[]' value='{{$regulstock->UniteId}}' type='number'></td>
                                <td><input class='text-right form-control' list='PUnite{{$regulstock->UniteId}}{{$regulstock->ProduitId}}' name='Qte[]' min='0' value="{{$regulstock->Qte}}" type='number' required /></td>
                                <td>
                                    @if($regulstock->Entree==1)
                                    <input class='checkbox' list='PUnite{{$regulstock->UniteId}}{{$regulstock->ProduitId}}' name='Entree[]'  checked type='checkbox' />
                                @else
                                <input class='checkbox' list='PUnite{{$regulstock->UniteId}}{{$regulstock->ProduitId}}' name='Entree[]' type='checkbox' />
                                @endif
                                </td>
                                <td style='text-align:center;'><button class='btn btn-danger btn-sm remove_this' style='width:25px;height:25px;border-radius:100px;' type='button' name='PUnite{{$regulstock->UniteId}}{{$regulstock->ProduitId}}'><span class='fa fa-trash'></span></button></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
