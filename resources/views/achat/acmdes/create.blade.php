...
{!! Form::open(array('route' => 'acmdes.store','method'=>'POST')) !!}
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3">
                        ...
                        <div>
                            <select name="FournisseurId" id="FournisseurId">
                                ...
                                <option value="{{$fournisseur->id}}" {{ (old('FournisseurId')==$fournisseur->id) ?
                                    'selected' : ''}}>
                                    {{$fournisseur->Nom}}
                                </option>
                                @endforeach
    ...
    $(document).ready(function() {
        ...
        $("#navMenus").on('click', 'li.selector', function() {
            ...
            if (produitId != "") {
                ...
                $.ajax({
                    url: "{{url('achat/acmdes/getUnites')}}",
                    method: 'POST',
                    dataType: 'json',
                    data: {
                        id: produitId
        ...
        function getFrAvoir() {
            ...
            if (FournisseurId != "") {
                $.ajax({
                    url: "{{url('achat/acmdes/getFrAvoir')}}",
                    method: 'POST',
                    dataType: 'json',
                    data: {
                        id: FournisseurId
