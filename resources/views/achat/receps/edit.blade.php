                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="form-group row">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <input class="form-control" name="Reference" value="{{old('Reference',$recep->Reference)}}" type="text">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input class="form-control datepicker" name="DateReception" value="{{old('DateReception',\Carbon\Carbon::parse($recep->DateReception)->format('d/m/Y'))}}" type="text" required>
                                </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="overflow-y: scroll;max-height:200px;">
            <table class="table table-bordered" id="TableOfData" style="font-size:80%;">
                <tr style="background:#006fcf;color:#FFF;font-weight:bold;">
                    ...
                    <td>Produit</td>
                    <td>Unité</td>
                    <td class="text-right innerTd">Qté à recevoir</td>
                ...
                <tbody id="DetailsUnites">
                    ...
                    @foreach ($cmde->produits->where('id',$recep_produit->id) as $cmde_produit)

                    @if($recep_produit->pivot->UniteId==$cmde_produit->pivot->UniteId)

                    @php
                    $Quantite= $cmde_produit->pivot->Qte - $cmde_produit->pivot->QteReçu + $recep_produit->pivot->Qte;
                    @endphp

                    @if($Quantite!='0')
                    <tr class='PUnite{{$recep_produit->pivot->UniteId}}{{$recep_produit->id}}'>
                        <td>{{$recep_produit->Libelle}}</td>
                        <td>{{$recep_produit->unites->where('id',$recep_produit->pivot->UniteId)->first()->Nom}}</td>
                        <td hidden><input class='text-right form-control' name='Produit[]' value='{{$recep_produit->id}}' type='number'></td>
                        <td hidden><input class='text-right form-control' name='Unite[]' value='{{$recep_produit->pivot->UniteId}}' type='number'></td>
                        <td><input class='text-right form-control' list='PUnite{{$recep_produit->pivot->UniteId}}{{$recep_produit->id}}' name='Qte[]' min='0' max='{{$Quantite}}' value='{{$Quantite}}' type='number' required /></td>
                        <td style='text-align:center;'><button class='btn btn-danger btn-sm remove_this' style='width:25px;height:25px;border-radius:100px;' type='button' name='PUnite{{$recep_produit->pivot->UniteId}}{{$recep_produit->id}}'><span class='fa fa-trash'></span></button></td>
                    </tr>
                    @endif
                    @break
                    @endif
