        ...
        <div class="col-sm-12 col-xs-12 col-md-6 col-lg-6">
            ...
            <div class="btn-group dropleft" style="float: right;">
                ...
                <ul class="dropleft dropdown-menu dropdown-inverse">
                    @can('createvlivr')
                    <li>
                        <a href="{{ route('livrs.create') }}" style="vertical-align: inherit;"><span class="glyphicon glyphicon-plus blue"></span> Nouvelle livraison</a>
                    </li>
                    @endcan
                        ...
                        <table class="table table-bordered table-stripped data-table">
                            <thead>
                                <tr>
                                    ...
                                    <th>Client</th>
                                    <th>Commande</th>
                                    <th class="text-right">Montant livraison</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $row->Reference }}</td>
                                    <td>@if( $row->commande) {{ $row->commande->client->Nom }} @endif</td>
                                    <td>@if( $row->commande){{ $row->commande->Reference }} @endif</td>
                                    <td class="text-right">{{number_format($row->MontantLivre,0,',',' ') }}</td>
                                    <td class="text-center">@if ($row->MontantLivre == $row->MontantFacture)
                                            ...
                                            <ul class="dropleft dropdown-menu dropdown-inverse">
                                                <li>
                                                    <p style="min-width:250px;margin:0 0 0px;">
                                                        <a href="{{ route('livrs.show',$row->id) }}" style="padding-bottom:4px;" class="btn btn-white btn-xs bolder"><span class="ace-icon fa fa-eye dark"></span> Aperçu</a>

                                                        @can('editvlivr')

                                                        <a href="{{ route('livrs.edit',$row->id) }}" style="padding-bottom:0px;" class="btn btn-warning btn-xs bolder"><span class="glyphicon glyphicon-edit"></span>
                                                            Modifier</a>

                                                        @endcan
                                                        @if ($row->MontantLivre != $row->MontantFacture)
                                                        @can('createvfact')

                                                        <a href="{{ route('vfacts.addfact',$row->id) }}" style="padding-bottom:0px;" class="btn btn-info btn-xs bolder"><span class="icon-books"></span>
                                                            Facture</a>
...
<script type="text/javascript">
    ...
    $('.show_confirm').click(function(event) {
        ...
        $("#SubmitForm").attr('action', '/vente/livrs/' + id);

        event.preventDefault();
        swal({
                title: `Etes-vous sûr de vouloir supprimer la livraison: ` + name + '?',
