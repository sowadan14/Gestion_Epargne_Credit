                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="form-group row">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <input class="form-control" name="Reference" value="{{old('Reference',$livr->Reference)}}" type="text">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input class="form-control datepicker" name="DateLivraison" value="{{old('DateLivraison',\Carbon\Carbon::parse($livr->DateLivraison)->format('d/m/Y'))}}" type="text" required>
                                </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="overflow-y: scroll;max-height:200px;">
            <table class="table table-bordered" id="TableOfData" style="font-size:80%;">
                ...
                <tbody id="DetailsUnites">
                    ...
                    @foreach ($cmde->produits->where('id',$livr_produit->id) as $cmde_produit)
                    ...
                    <tr class='PUnite{{$livr_produit->pivot->UniteId}}{{$livr_produit->id}}'>
                        ...
                        <td>{{$livr_produit->unites->where('id',$livr_produit->pivot->UniteId)->first()->Nom}}</td>
                        <td hidden><input class='text-right form-control' name='Produit[]' value='{{$livr_produit->id}}' type='number'></td>
                        <td hidden><input class='text-right form-control' name='Unite[]' value='{{$livr_produit->pivot->UniteId}}' type='number'></td>
                        <td><input class='text-right form-control' list='PUnite{{$livr_produit->pivot->UniteId}}{{$livr_produit->id}}' name='Qte[]' min='0' max='{{$Quantite}}' value='{{$Quantite}}' type='number' required /></td>
                        <td style='text-align:center;'><button class='btn btn-danger btn-sm remove_this' style='width:25px;height:25px;border-radius:100px;' type='button' name='PUnite{{$livr_produit->pivot->UniteId}}{{$livr_produit->id}}'><span class='fa fa-trash'></span></button></td>
                    </tr>
                    @endif
                    @break
                    @endif
