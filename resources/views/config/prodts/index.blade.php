			...
			<div class="col-sm-12 col-xs-12 col-md-6 col-lg-6">
				...
				<div class="btn-group dropleft" style="float: right;">
					...
					<ul class="dropleft dropdown-menu dropdown-inverse">
						@can('createproduit')
						<li>
							<a href="{{ route('prodts.create') }}" style="vertical-align: inherit;"><span class="glyphicon glyphicon-plus blue"></span> Nouveau produit</a>
						</li>
						@endcan
							...
							<table class="table table-bordered table-stripped data-table">
								...
								<tbody>
									...
									<tr>
										...
										<td>
										@if ($row->Prod_logo)
                                            <img id="blah" src="{{ asset('storage/images/'.$row->Prod_logo) }}"
                                                alt="User photo" width="20px" height="20px"
										...
										<td>{{ $row->Libelle }}</td>
										<td>{{ $row->CodeBar }}</td>
										<td>{{ isset($row->typeproduit) ? $row->typeproduit->Nom:''}}</td>
										<td>{{ isset($row->typeproduit) ? $row->unite->Nom:''}}</td>
										<td class="text-right">{{ $row->CUMP }}</td>
												...
												<ul class="dropleft dropdown-menu dropdown-inverse">
													<li>
														<p style="min-width:250px;margin:0 0 0px;">
															<a href="{{ route('prodts.show',$row->id) }}" style="padding-bottom:4px;" class="btn btn-white btn-xs bolder"><span class="ace-icon fa fa-eye dark"></span> Aperçu</a>

															@can('editproduit')

															<a href="{{ route('prodts.edit',$row->id) }}" style="padding-bottom:0px;" class="btn btn-warning btn-xs bolder"><span class="glyphicon glyphicon-edit"></span>
																Modifier</a>
															...
															<!-- {!! Form::open(['method' => 'DELETE','route' => ['prodts.destroy',
