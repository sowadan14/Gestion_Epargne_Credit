			...
			<div class="col-sm-12 col-xs-12 col-md-6 col-lg-6">
				...
				<div class="btn-group dropleft" style="float: right;">
					...
					<ul class="dropleft dropdown-menu dropdown-inverse">
						@can('createcompte')
						<li>
							<a href="{{ route('comptes.create') }}" style="vertical-align: inherit;"><span class="glyphicon glyphicon-plus blue"></span> Nouveau compte</a>
						</li>
						@endcan
							...
							<table class="table table-bordered table-stripped data-table">
								<thead>
									<tr>
										...
										<th>Code</th>
										<th>Libellé</th>
										<th class="text-right">Solde Intial</th>
								...
								<tbody>
									...
									<tr>
										...
										<td>{{ $row->Code }}</td>
										<td>{{ $row->Libelle }}</td>
										<td class="text-right">{{number_format($row->SoldeInitial,0,',',' ') }}</td>
												...
												<ul class="dropleft dropdown-menu dropdown-inverse">
													<li>
														<p style="min-width:250px;margin:0 0 0px;">
															<a href="{{ route('comptes.show',$row->id) }}" style="padding-bottom:4px;" class="btn btn-white btn-xs bolder"><span class="ace-icon fa fa-eye dark"></span> Aperçu</a>

															@can('editcompte')

															<a href="{{ route('comptes.edit',$row->id) }}" style="padding-bottom:0px;" class="btn btn-warning btn-xs bolder"><span class="glyphicon glyphicon-edit"></span>
																Modifier</a>
															...
															<!-- {!! Form::open(['method' => 'DELETE','route' => ['comptes.destroy',
...
<script type="text/javascript">
	$('.show_confirm').click(function(event) {
		...
		$("#SubmitForm").attr('action', '/config/comptes/' + id);
...
<script type="text/javascript">
	$(function() {
		...
		$('#addcompte').click(function() {
			...
			$('#modelHeading').html("<i class='glyphicon glyphicon-plus'></i> Nouveau compte");
		...
		--------------------------------------------
		--------------------------------------------*/
		$('body').on('click', '.editcompte', function() {
			jQuery('.alert-danger').html('');

			var compte_id = $(this).data('id');
			$.get("{{ route('comptes.index') }}" + '/' + compte_id + '/edit', function(data) {
				$('#modelHeading').html("<i class='glyphicon glyphicon-edit'></i> Modification d'un compte");
		...
		$('body').on('click', '.showcompte', function() {
			jQuery('.alert-danger').html('');

			var compte_id = $(this).data('id');
			$.get("{{ route('comptes.index') }}" + '/' + compte_id + '/edit', function(data) {
				$('#showModelHeading').html("<i class='ace-icon fa fa-eye'></i> Details d'un compte");
		...
		$('#CreateForm').click(function(e) {
			e.preventDefault();
			$.ajax({
				data: $('#compteForm').serialize(),
				url: "{{ route('comptes.store') }}",
				type: "POST",
		...
		$('body').on('click', '.deletecompte', function() {
			var id = $(this).data('id');
			var Libelle = $(this).attr('role');
			$('#deleteModelHeading').html("<i class='ace-icon fa fa-trash-o bigger-120'></i> Suppression d'un compte");
			$('#deleteAjaxModel').modal('show');
			$("#deletecompteid").val(id);
			$("#deleteForm").attr('action', '/comptes/' + id);
			$('.forDelete').html("Voulez-vous vraiment supprimer le compte: " + Libelle + "?");
